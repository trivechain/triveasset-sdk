...
var HDWallet = function (settings) {
  ...
  if (settings.network === 'testnet') {
    ...
    self.network = bitcoin.networks.trivecointestnet
  } else {
    settings.blockExplorerHost = settings.blockExplorerHost || mainnetBlockExplorerHost
    self.network = bitcoin.networks.trivecoin
  }
...
HDWallet.createNewKey = function (network, pass, progressCallback) {
  ...
  network = (network === 'testnet' ? bitcoin.networks.trivecointestnet : bitcoin.networks.trivecoin)
  ...
  // coin_type'
  node = node.deriveHardened(network === bitcoin.networks.trivecoin ? 0 : 1)
  // account'
...
HDWallet.getInputAddresses = function (txHex, network) {
  network = network || bitcoin.networks.trivecoin
  var addresses = []
  var tx
  try {
...
HDWallet.prototype.getKeyPrefix = function () {
  ...
  var network = (self.network === bitcoin.networks.trivecoin) ? 'mainnet' : 'testnet'
...
HDWallet.prototype.registerAddress = function (address, accountIndex, addressIndex, change) {
  ...
  var coinType = self.network === bitcoin.networks.trivecoin ? 0 : 1
  change = (change) ? 1 : 0
...
HDWallet.prototype.deriveAccount = function (accountIndex) {
  ...
  if (this.preAccountNode) {
    ...
    node = node.deriveHardened(44)
    // coin_type'
    node = node.deriveHardened(this.network === bitcoin.networks.trivecoin ? 0 : 1)
